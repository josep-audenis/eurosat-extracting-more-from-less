import sys
import os
import numpy as np
#import cudf

#from cuml.ensemble import RandomForestClassifier as cuRF
from sklearn.preprocessing import LabelEncoder

sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..")))

from src.validations.cross_validation import cross_validate_model
from src.models.model_utils import load_features

DATASET_PATH = os.path.join(os.path.dirname(__file__), "../..",  "data/interim/")


def train_random_forest_cv_gpu(dataset_name, n_splits=5):


    dataset_filename = DATASET_PATH + dataset_name

    X, y = load_features(dataset_filename)

    #X_train_cudf = cudf.DataFrame.from_records(X)
    #y_train_cudf = cudf.Series(y)

    le = LabelEncoder()
    y_encoded = le.fit_transform(y)

    #model = cuRF(n_estimators=100, random_state=42)

    #results, fold_metrics = cross_validate_model(X, y_encoded, model, n_splits=n_splits, random_seed=42)

    # Saving model ???


if __name__ == "__main__":
    dataset_filename = input("What dataset would you like to use: ")
    print(os.listdir(DATASET_PATH))

    if dataset_filename in os.listdir(DATASET_PATH):
        print(".npz")
        train_random_forest_cv_gpu(dataset_filename)
    elif dataset_filename + ".npz" in os.listdir(DATASET_PATH):
        print("no .npz")
        train_random_forest_cv_gpu(dataset_filename + ".npz")
        
    else:
        print(f"No {dataset_filename} or {dataset_filename}.npz found in {DATASET_PATH}")
        exit